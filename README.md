...
#### `denied to github-actions[bot]` と表示された場合

このエラーは `SYNC_TOKEN` ではなく、checkout 時に設定された同期元の `GITHUB_TOKEN`
で同期先へ push しようとした場合に発生します。本ワークフローでは checkout の
`persist-credentials: false` により `GITHUB_TOKEN` を Git 設定へ残さず、push 時に
`SYNC_TOKEN` が使われるようにしています。修正を含む `main` でワークフローを再実行
してください。

修正後にエラーが `denied to <ユーザー名>` に変わった場合は `SYNC_TOKEN` 自体は使用
されています。その場合は、トークンの **Resource owner**、対象リポジトリ、
**Contents: Read and write**、Organization の承認状態、および同期先 ruleset の
force push 設定を確認してください。
